...
public interface IReservation : IGeneric<Models.Reservation>
{
    Task<ReservationDTO> CreateReservationAsync(CreateReservationDto createDto);
    Task<ReservationDTO> CreateReservationForOrganizationAsync(
        CreateReservationForOrganizationDto createDto
    );
    ...
    Task<GetReservationsByResourcesResponse> GetReservationsByResourcesAsync(
        GetReservationsByResourcesRequest request
    );

    Task<IEnumerable<DateReservationCountDTO>> GetReservationsCountPerDayAsync(
    ...
    Task<SearchClientsResponse> SearchClientsByNameAsync(
        string nameQuery,
        string organizationId,
        int maxResults
    );

    Task<GetReservationsBySourceCountResponse> GetReservationsBySourceCountAsync(
        string organizationId,
        DateTime startDate,
        DateTime endDate
    );

    Task<byte[]> GenerateReservationReportAsync(
        string organizationId,
        string startDate,
        string endDate
    );
